...
Now that we have preprocessed the data, let's begin applying the functions from the **FinalProjectGroup13** package to perform logistic regression analysis on the data. We will use the **smoker** and **bmi** columns for this analysis.
...
expenses$bmi <- as.numeric(expenses$bmi)
expenses$children <- as.numeric(expenses$children)

# Create the predictor matrix (bmi and children columns) as a numeric matrix
X <- as.matrix(expenses[, c("bmi", "children")])
